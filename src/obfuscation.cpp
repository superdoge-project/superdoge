...
/* *** BEGIN OBFUSCATION MAGIC - SUPERDOGE **********
    ...
    if (fMasterNode) {
        ...
        for (const CTransaction& txCollateral : vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat SUPERDOGE and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 SUPERDOGE per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeSUperdogeAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100SUPERDOGE+1 ( bit on if present )
    // bit 1 - 10SUPERDOGE+1
    // bit 2 - 1SUPERDOGE+1
    // bit 3 - .1SUPERDOGE+1
    // bit 3 - non-denom
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("superdoge-obfuscation");
