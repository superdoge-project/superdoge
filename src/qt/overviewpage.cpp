...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::SDOGE)
    {
    }
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->label_SUPERDOGE4->setVisible(showImmature || showWatchOnlyImmature);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    if (model && model->getOptionsModel()) {
        ...
        connect(ui->blabel_SUPERDOGE, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

    }

    // update the display unit, to not use the default ("SUPERDOGE")
    updateDisplayUnit();
}
...
void OverviewPage::updatBlockChainInfo()
{
    ...
    double BlockRewardSUPERDOGE =  static_cast<double>(BlockReward)/static_cast<double>(COIN);

    ui->label_CurrentBlock_value->setText(QString::number(CurrentBlock));

    ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardSUPERDOGE));
}
