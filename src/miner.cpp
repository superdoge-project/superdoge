...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SuperDogeMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("SuperDogeMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("SuperDogeMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("superdoge-miner");
    ...
    //control the amount of times the client will check for mintable coins
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("SuperDogeMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running SuperDogeMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
